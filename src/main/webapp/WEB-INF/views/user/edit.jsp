	...
	<div class="container">
		<div class="row mt-4">
			<div class="col-5 m-auto">
				...
				<form action="<%=request.getContextPath()%>/user/add"
					method="POST">
					...
					<div class="form-group">
						<label>Họ tên</label> <input type="text"
							class="form-control" name="name" value="${ user.fullname }"/>
					</div>
					<div class="form-group">
						<label>Avatar</label> <input type="text"
							class="form-control" name="avatar" value="${ user.avatar }"/>
					</div>
					<div class="form-group">
						<label>Quyền</label><select class="form-control"
							...
							<c:forEach items="${ listRole }" var="role">
								<c:choose>
									<c:when test="${ user.roleDto.id == role.id }">
					...
					<a href="<%= request.getContextPath() %>/user" class="btn btn-secondary">Quay lại</a>
				</form>
			</div>
		</div>
	</div>
