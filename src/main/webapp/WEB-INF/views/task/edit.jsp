	...
	<div class="container">
		<div class="row mt-4">
			<div class="col-5 m-auto">
				<h2 class="mb-4">Cập nhật đầu việc</h2>
				...
				<form action="<%=request.getContextPath()%>/task/edit" method="POST">
					...
					<div class="form-group">
						<label>Tên đầu việc</label> <input type="text"
							class="form-control" name="name" value="${ task.name }"/>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						<label>Công việc</label><select class="form-control"
							...
							<c:forEach items="${ listProject }" var="project">
								<c:choose>
									<c:when test="${ task.projectDto.id == project.id }">
					...
					<div class="form-group">
						<label>Trạng thái</label><select class="form-control"
							...
							<c:forEach items="${ listStatus }" var="status">
								<c:choose>
									<c:when test="${ task.statusDto.id == status.id }">
					...
					<div class="form-group">
						<label>Người dùng</label><select class="form-control"
							...
							<c:forEach items="${ listUser }" var="user">
								<c:choose>
									<c:when test="${ task.userDto.id == user.id }">
